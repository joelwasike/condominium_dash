...
const AccountingDashboard = () => {
  ...
  const [selectedItemForView, setSelectedItemForView] = useState(null); // { type: 'collection'|'payment', data }
  ...
  const [selectedTenant, setSelectedTenant] = useState(null); // tenant object from accounting list (no sales manager API)
  ...
  // Download receipt - use PDF if URL exists, otherwise generate SAAF IMMO format PDF
  const downloadReceipt = async (item) => {
    if (!item) return;
    try {
      // If receipt URL exists (tenant payment), download it directly
      ...
      // Generate PDF in SAAF IMMO format (same as BON DE CAISSE)
  ...
  // Helper: number to words in French (shared for receipts)
  ...
  // Tenant detail view - uses accounting data only (no sales manager API)
  ...
  // Get filtered expenses (all filters applied before display and export)
