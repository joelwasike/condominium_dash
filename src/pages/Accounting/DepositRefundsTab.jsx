      ...
      {showDepositPaymentModal && (
        <div className="modal-overlay" onClick={() => setShowDepositPaymentModal(false)}><div className="modal-content" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="modal-body">
            <form onSubmit={async (e) => { e.preventDefault(); try { setLoading(true); await accountingService.recordDepositPayment({...depositPaymentForm, monthlyRent: parseFloat(depositPaymentForm.monthlyRent), amount: parseFloat(depositPaymentForm.monthlyRent) * 4.5, monthsMultiplier: 4.5}); addNotification('Security deposit payment recorded successfully!', 'success'); setShowDepositPaymentModal(false); setDepositPaymentForm({tenant:'',property:'',tenantType:'individual',monthlyRent:'',paymentMethod:'mobile_money',reference:'',notes:''}); await loadDeposits(); loadDepositRefundsPending(); } catch (error) { console.error('Error recording deposit payment:', error); addNotification(error.message || 'Failed to record deposit payment', 'error'); } finally { setLoading(false); } }}>
              <div className="form-group"><label>Tenant Name *</label><select value={depositPaymentForm.tenant} onChange={(e) => { const name = e.target.value; const t2 = tenants.find(x => (x.tenantName || x.TenantName) === name); setDepositPaymentForm({...depositPaymentForm, tenant: name, property: t2 ? (t2.property || t2.Property || '') : ''}); }} required><option value="">Select tenant (with property)</option>{tenants.filter(t2 => (t2.property || t2.Property)).map((t2) => { const name = t2.tenantName || t2.TenantName || ''; return <option key={name} value={name}>{name} - {t2.property || t2.Property}</option>; })}</select></div>
              ...
              <div className="form-group"><label>Monthly Rent (XOF) *</label><input type="number" step="0.01" value={depositPaymentForm.monthlyRent} onChange={(e) => setDepositPaymentForm({...depositPaymentForm, monthlyRent: e.target.value})} required placeholder="0.00" />{depositPaymentForm.monthlyRent && <small style={{ color: '#059669', marginTop: '4px', display: 'block', fontWeight: '600' }}>Deposit Amount: {(parseFloat(depositPaymentForm.monthlyRent) * 4.5).toFixed(2)} XOF</small>}</div>
