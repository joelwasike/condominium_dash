      ...
      {showDepositPaymentModal && (
        <div className="modal-overlay" onClick={() => setShowDepositPaymentModal(false)}><div className="modal-content" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="modal-body">
            <form onSubmit={async (e) => { e.preventDefault(); try { setLoading(true); await accountingService.recordDepositPayment({...depositPaymentForm, monthlyRent: parseFloat(depositPaymentForm.monthlyRent), amount: parseFloat(depositPaymentForm.monthlyRent) * 4.5 + (depositPaymentForm.applicationFees ? 37000 : 0), monthsMultiplier: 4.5}); addNotification('Security deposit payment recorded successfully!', 'success'); setShowDepositPaymentModal(false); setDepositPaymentForm({tenant:'',property:'',tenantType:'individual',monthlyRent:'',applicationFees:false,paymentMethod:'mobile_money',paymentProvider:'',reference:'',notes:''}); await loadDeposits(); loadDepositRefundsPending(); } catch (error) { console.error('Error recording deposit payment:', error); addNotification(error.message || 'Failed to record deposit payment', 'error'); } finally { setLoading(false); } }}>
              <div className="form-group"><label>Tenant Name *</label><input list={depositTenantListId} value={depositPaymentForm.tenant} onChange={(e) => { const name = e.target.value; const t2 = tenantList.find(x => (x.tenantName || x.TenantName || x.name || x.Name || '').toLowerCase() === name.toLowerCase()); const appFees = t2 ? !!(t2.applicationFees ?? t2.ApplicationFees) : false; setDepositPaymentForm({...depositPaymentForm, tenant: name, property: t2 ? (t2.property || t2.Property || '') : '', applicationFees: appFees}); }} required placeholder="Type to search approved tenant" /><datalist id={depositTenantListId}>{tenantList.filter(t2 => (t2.property || t2.Property)).map((t2) => { const name = t2.tenantName || t2.TenantName || t2.name || t2.Name || ''; return <option key={t2.tenantId ?? t2.TenantID ?? name} value={name}>{name} - {t2.property || t2.Property}</option>; })}</datalist></div>
