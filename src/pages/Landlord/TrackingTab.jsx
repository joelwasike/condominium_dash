    ...
    <div className="sa-overview-page">
      ...
      <div className="sa-section-card" style={{ marginBottom: '24px' }}>
        ...
        <div style={{ width: '100%', height: '300px', padding: '20px' }}>
          {chartData.length > 0 ? <ResponsiveContainer><AreaChart data={chartData} margin={{ top: 10, right: 30, left: 20, bottom: 10 }}><defs><linearGradient id="colorRevenueTracking" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#3b82f6" stopOpacity={0.3} /><stop offset="95%" stopColor="#3b82f6" stopOpacity={0} /></linearGradient><linearGradient id="colorExpensesTracking" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#f59e0b" stopOpacity={0.3} /><stop offset="95%" stopColor="#f59e0b" stopOpacity={0} /></linearGradient></defs><CartesianGrid strokeDasharray="3 3" stroke="#e5e7eb" opacity={0.5} /><XAxis dataKey="month" stroke="#6b7280" tick={{ fill: '#6b7280', fontSize: 12 }} axisLine={{ stroke: '#e5e7eb' }} /><YAxis stroke="#6b7280" tick={{ fill: '#6b7280', fontSize: 12 }} axisLine={{ stroke: '#e5e7eb' }} tickFormatter={(v) => `${(v / 1000).toFixed(0)}k`} /><Tooltip contentStyle={{ backgroundColor: 'rgba(255, 255, 255, 0.95)', border: '1px solid #e5e7eb', borderRadius: '8px', boxShadow: '0 4px 6px rgba(0, 0, 0, 0.1)', padding: '8px 12px' }} formatter={(value) => [`${Number(value).toLocaleString()} XOF`, '']} labelFormatter={(l) => `Month: ${l}`} /><Legend wrapperStyle={{ paddingTop: '10px' }} iconType="line" /><Area type="monotone" dataKey="revenue" stroke="#3b82f6" fill="url(#colorRevenueTracking)" name="Revenue" strokeWidth={3} dot={{ fill: '#3b82f6', r: 5, strokeWidth: 2, stroke: '#fff' }} activeDot={{ r: 7, strokeWidth: 2, stroke: '#fff' }} /><Area type="monotone" dataKey="expenses" stroke="#f59e0b" fill="url(#colorExpensesTracking)" name="Expenses" strokeWidth={3} dot={{ fill: '#f59e0b', r: 5, strokeWidth: 2, stroke: '#fff' }} activeDot={{ r: 7, strokeWidth: 2, stroke: '#fff' }} /></AreaChart></ResponsiveContainer> : <div style={{ textAlign: 'center', padding: '80px', color: '#9ca3af' }}>No chart data available yet</div>}
        </div>
      </div>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '24px' }}>
        <div className="sa-section-card" style={{ flex: '1 1 400px', minWidth: '300px' }}>
          ...
          <div style={{ width: '100%', height: '280px', padding: '20px' }}>{chartData.length > 0 ? <ResponsiveContainer><BarChart data={chartData}><CartesianGrid strokeDasharray="3 3" stroke="#e5e7eb" opacity={0.5} /><XAxis dataKey="month" stroke="#6b7280" tick={{ fill: '#6b7280', fontSize: 12 }} axisLine={{ stroke: '#e5e7eb' }} /><YAxis domain={[0, 100]} stroke="#6b7280" tick={{ fill: '#6b7280', fontSize: 12 }} axisLine={{ stroke: '#e5e7eb' }} tickFormatter={(v) => `${v}%`} /><Tooltip contentStyle={{ backgroundColor: 'rgba(255, 255, 255, 0.95)', border: '1px solid #e5e7eb', borderRadius: '8px', boxShadow: '0 4px 6px rgba(0, 0, 0, 0.1)', padding: '8px 12px' }} formatter={(value) => [`${Number(value).toFixed(1)}%`, 'Occupancy']} /><Bar dataKey="occupancy" fill="#10b981" name="Occupancy %" radius={[4, 4, 0, 0]} /></BarChart></ResponsiveContainer> : <div style={{ textAlign: 'center', padding: '80px', color: '#9ca3af' }}>No data</div>}</div>
        </div>
        <div className="sa-section-card" style={{ flex: '1 1 400px', minWidth: '300px' }}>
